...
- **State machine** lives in `src/timer.ts` (`computeState`) and is driven by a 1-second `setInterval` tick in `src/main.ts`. States: `awaiting_capture` (0 posts) → `feed` (1 post, can post again) → `before_trigger` (2 posts, countdown to next trigger). There is no trigger-time gate for users with 0 posts — any user who hasn't posted this period goes directly to capture. The `before_trigger` countdown fires only after the quota (`MAX_POSTS_PER_TRIGGER`) is reached; the tick also detects when a new period starts in-session (via `getLastTriggerTime()` changing) and resets `periodPostCount` to 0 without requiring a page reload. Post count is fetched from the server on every page load (`fetchTodayPostCount`, anchored to `getLastTriggerTime()`) and held in the module-level `periodPostCount` variable in `main.ts`; no localStorage cache is used for the count.
