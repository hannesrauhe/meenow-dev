...
- **localStorage keys** follow the `meenow:*` namespace (see `src/state.ts`): `meenow:install-dismiss`, `meenow:notif-dismiss`, `meenow:push-sub-file`, `meenow:pwa-subbed`. The push-sub-file key stores the subscription filename written to the relay repo, preventing duplicate registrations from the same device. The pwa-subbed key records that the active subscription was created in standalone mode (so Chrome routes notifications to the PWA, not to Chrome).
